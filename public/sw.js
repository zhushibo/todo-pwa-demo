 ...
 * @LastEditTime: 2019-07-02 10:44:38
 * @LastEditors: doctor
 */
var cacheName = 'bs-0-2-1';

var apiCacheName = 'api-0-1-4';
var cacheFiles = [
    '/',
    './index.js',
    './favicon.ico',
...
self.addEventListener('fetch', function (e) {
    ...
    let requestUrl = getUrlRelativePath(e.request.url)
    // 判断当前请求是否需要缓存
    ...
    if (needCache) {
        ...
        caches.open(apiCacheName).then(function (cache) {
            console.log('缓存接口',requestUrl)
            return fetch(e.request).then(function (response) {
                cache.put(e.request.url, response.clone());
                console.log('缓存api',response)
                return response;
            });
        });
    }
...
//                 self.clients.openWindow && self.clients.openWindow('http://127.0.0.1:3002');
//                 return;
//             }
